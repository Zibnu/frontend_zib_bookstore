...
function Transaction() {
    ...
    try {
      ...
      const updateOrders = await Promise.all(
        orderData.map( async (order) => {
          try {
            const shipRes = await apiServices.get(`/shipment/log-order/${order.id_order}`,
              { headers : { Authorization : `Bearer ${token}`}}
            );
  ...
  if(loading) return <div className="text-center py-10 text-gray-600">Memuat Transaksi....</div>
